...
入口地址是运行时数据，不作为编译期常量：启动器每次启动读取4399官方游戏页（`www.4399.com/flash/130396.htm`）中当前发布的包装页路径，只接受HTTPS、4399主机和`upload_swf`下版本化文件；官方页给的是`sda`主机，它对无Referer请求返回错误页（防盗链），因此把主机换成同路径的可直连主机后交给原生宿主。解析失败时依次回退上次成功解析的缓存（`game-page.json`）和随包兜底地址。版本号写在包装页文件名里（`v3690g.htm`对应`v3690.swf`），游戏更新后旧版地址会下线，因此不能长期固定某一版。原生登录门禁同样按来源和路径前缀匹配，不绑定具体版本文件名。
